...
static uint8_t previous_connect_attempt;
...
static void mqtt_event(struct mqtt_connection *m, mqtt_event_t event, void *data) {
  ...
  case MQTT_EVENT_PUBLISH: {
    ...
    if(msg_ptr->first_chunk) {
      ...
      LOG_INFO("Application received a publish on topic '%s'; payload size is %i bytes\n", msg_ptr->topic, msg_ptr->payload_length);
    }
  ...
  case MQTT_EVENT_PUBACK: {
    LOG_INFO("Received ack of a previous publish.\n");
    break;
  }
  default:
...
static void publish(void)
{
  ...
  LOG_INFO("Executed a publish on \"%s\" with message \"%s\".\n", pub_topic, app_buffer);
}
...
static void state_machine(void) {
  ...
  case STATE_CONNECTING:
    ...
    if(connect_attempt != previous_connect_attempt) {
      LOG_INFO("Connecting: retry %u...\n", connect_attempt);
      previous_connect_attempt = connect_attempt;
    }
    break;
  ...
  case STATE_PUBLISHING:
    ...
    if(mqtt_ready(&conn) && conn.out_buffer_sent) {
      ...
      return; // Return here so we don't end up rescheduling the timer.
    } else {
      /*
       ...
       */
      LOG_INFO("A packet is already in queue... (MQTT state=%d, q=%u)\n", conn.state, conn.out_queue_full);
    }
    break;
